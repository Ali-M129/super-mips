...
## Task 6 shared-memory contract

The dual backend exposes exactly one data-memory request per cycle. Lane 0 has
normal priority. A defensive two-request condition is serialized as lane 0 on
the first cycle and lane 1 on the second cycle, with E/M held between phases.
Memory side effects are suppressed whenever E/M cannot advance.
